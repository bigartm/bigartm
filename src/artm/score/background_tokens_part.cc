...
std::shared_ptr<Score> BackgroundTokensPart::CalculateScore(const artm::core::PhiMatrix& p_wt) {
  ...
  if (delta_threshold < 0) {
    BOOST_THROW_EXCEPTION(artm::core::ArgumentOutOfRangeException(
      "BackgroundTokensPartScoreConfig.delta_threshold",
  ...
  // count score
  auto btp_score = new BackgroundTokensPartScore();
