...
bool Helpers::Validate(const ::artm::InitializeModelArgs& message, bool throw_error) {
  ...
  if (message.has_source_type() || message.has_disk_path() ||
      message.has_filter() || message.has_batch_filename()) {
...
bool Validate(const ::artm::CollectionParserConfig& message, bool throw_error = true) {
  std::stringstream ss;

  if (message.has_cooccurrence_token() || message.has_gather_cooc() ||
      message.has_cooccurrence_class_id() || message.has_use_symmetric_cooc_values()) {
