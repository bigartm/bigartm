...
bool ImproveCoherencyPhi::RegularizePhi(const ::artm::core::Regularizable& topic_model,
  ...
  if (!config_.has_dictionary_name()) {
    LOG(WARNING) << "There's no dictionary for ImproveCoherency regularizer. Cancel it's launch.";
    return false;
  }
...
google::protobuf::RepeatedPtrField<std::string> ImproveCoherencyPhi::topics_to_regularize() {
  return config_.topic_name();
}

google::protobuf::RepeatedPtrField<std::string> ImproveCoherencyPhi::class_ids_to_regularize() {
  return config_.class_id();
}

bool ImproveCoherencyPhi::Reconfigure(const RegularizerConfig& config) {
  std::string config_blob = config.config();
  ImproveCoherencyPhiConfig regularizer_config;
  if (!regularizer_config.ParseFromArray(config_blob.c_str(), config_blob.length())) {
    BOOST_THROW_EXCEPTION(::artm::core::CorruptedMessageException(
      "Unable to parse ImproveCoherencyPhiConfig from RegularizerConfig.config"));
  }
