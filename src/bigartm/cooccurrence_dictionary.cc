...
void CooccurrenceDictionary::OpenExistingBatchFile(CooccurrenceBatch& batch) {
  ++open_files_counter_;
  batch.in_batch_.open(batch.filename_, std::ios::in);
}

void CooccurrenceDictionary::CloseExistingBatchFile(CooccurrenceBatch& batch) {
  --open_files_counter_;
...
void CooccurrenceDictionary::ReadAndMergeCooccurrenceBatches() {
  ...
  for (int i = 0; i < static_cast<int>(vector_of_batches_.size()); ++i) {
    OpenExistingBatchFile(*(vector_of_batches_[i]));
