...
model = ArtmModel(num_processors=2, document_passes_count=10,
                  topics_count = 7, class_ids={'@default_class' : 1.0})
...
model.regularizers.add(SmoothSparsePhiRegularizer('SmSpPhiReg', -0.1, ['@default_class']))
...
print len(retval.document_ids)
print model.scores_info['ItemScore'].value
model.save()
model.to_csv()
#vis = model.visualize(dictionary_path='kos/dictionary')
