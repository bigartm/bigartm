...
class LDA(object):
    def __init__(self, num_topics=None, num_processors=None, cache_theta=False,
                 dictionary=None, num_document_passes=10, seed=-1, alpha=0.01, beta=0.01):
        """
        :param int num_topics: the number of topics in model, will be overwrited if\
                                 topic_names is set
        :param int num_processors: how many threads will be used for model training, if\
        ...
        :param seed: seed for random initialization, -1 means no seed
        :type seed: unsigned int or -1
        :param double alpha: hyperparameter of Theta smoothing regularizer
        :param beta: hyperparameter of Phi smoothing regularizer
        :type beta: double or list of doubles with len == num_topics
        ...
        """
        if not isinstance(alpha, double) or not isinstance(alpha, int):
            raise ValueError('LDA.alpha should be double')

        if not isinstance(beta, list) or not isinstance(beta, double) or not isinstance(beta, int):
            raise ValueError('LDA.beta should be double or list of doubles')
        ...
        self._internal_model.regularizers.add(SmoothSparseThetaRegularizer(name=self._theta_reg_name, tau=alpha))
        if isinstance(beta, list):
            ...
            self._internal_model.regularizers.add(SmoothSparsePhiRegularizer(name=self._phi_reg_name, tau=beta))
        ...
        self._internal_model.scores.add(TopTokensScore(name=self._tt_score_name))
    ...
    def perplexity_value(self):
        return self._internal_model.score_tracker[_perp_score_name].value

    @property
    def perplexity_last_value(self):
        return self._internal_model.score_tracker[_perp_score_name].last_value

    @property
    def sparsity_phi_value(self):
        return self._internal_model.score_tracker[_sp_phi_score_name].value

    @property
    def sparsity_phi_last_value(self):
        return self._internal_model.score_tracker[_sp_phi_score_name].last_value

    @property
    def sparsity_theta_value(self):
        return self._internal_model.score_tracker[_sp_theta_score_name].value

    @property
    def sparsity_theta_last_value(self):
        return self._internal_model.score_tracker[_sp_theta_score_name].last_value
    ...
    def get_theta(self):
        ...
        :return:
          ...
          * data --- content of Theta matrix.
        """
        if self.internal_model.cache_theta:
    ...
    def get_top_tokens(self, num_tokens=10, with_weights=False):
        ...
        :return:
          * list of lists of str, each internal list corresponds one topic in\
            ...
            of tules, each tuple is (str, double)
        """
        self._internal_model.scores[self._tt_score_name].num_tokens = num_tokens
